...
export function generateNormals(maxAngle, positions, triangles) {
...
export function computeProjectedPlaneUVs(positions, projectedPlane = 'xy') {
  const idxs = projectedPlane2Ids[projectedPlane];
  const uvs = positions.map(function () {
    // Initialize to zero.
    return [0, 0];
  });

...
void BoxApp::BuildConstantBuffers() {
	ObjectCB = std::make_unique<UploadBuffer<ObjectConstants>>(md3dDevice.Get(), 1, true);
	MaterialCB = std::make_unique<UploadBuffer<MaterialConstants>>(md3dDevice.Get(), 1, true);
	PassCB = std::make_unique<UploadBuffer<PassConstants>>(md3dDevice.Get(), 1, true);
}

void BoxApp::BuildShadersAndInputLayout() {
	mvsByteCode = d3dUtil::CompileShader(L"Shaders\\default3.hlsl", nullptr, "VS", "vs_5_0");
	mpsByteCode = d3dUtil::CompileShader(L"Shaders\\default3.hlsl", nullptr, "PS", "ps_5_0");
...
void BoxApp::BuildFrameResources() {
    for (int i = 0; i < gNumFrameResources; ++i) {
        mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(), 1, 1, (UINT)mMaterials.size()));
    }
}
